...
The project includes several memory optimizations to reduce memory usage when running multiple MCP server instances:
- **Lazy model loading**: Embedding models are loaded on-demand when first used, not at startup
- **Model caching**: Models are cached and shared across instances within the same Python process (thread-safe singleton pattern)
- **Memory-mapped FAISS indexes**: For read-only search operations, indexes are loaded using FAISS file format which allows the OS to manage memory more efficiently. Indexes are automatically saved in both formats (serialized for backward compatibility and FAISS format for memory efficiency) during collection creation/updates.

**Note**: Existing collections will automatically benefit from memory-mapped indexes after the next update or recreation. The old serialized format is still supported for backward compatibility.
